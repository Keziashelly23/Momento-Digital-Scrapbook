...
// Save scrapbook content
async function saveToFirebase() {
  const user = auth.currentUser;
  if (!user) return alert("Please log in first!");

  const docRef = doc(db, "scrapbooks", user.uid);
  await setDoc(docRef, {
    content: rightPage.innerHTML,
    updatedAt: new Date()
  });

  console.log("✅ Scrapbook saved for user:", user.email);
}

// Load scrapbook content
async function loadFromFirebase() {
  ...
  if (docSnap.exists()) {
    rightPage.innerHTML = docSnap.data().content;
    console.log("📖 Scrapbook loaded for", user.email);
  } else {
    console.log("No scrapbook found, starting fresh!");
  }
}

// Load scrapbook when logged in
